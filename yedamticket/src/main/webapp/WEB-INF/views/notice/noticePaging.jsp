	...
	<div id="outter">
		...
		<table border="1">
			...
			<c:forEach items="${notices }" var="notice">
				<tr onclick="noticeSearch('${notice.noticeNo }')">
					...
					<td>${notice.title }</td>
					<td>${notice.writer }</td>
					<td><fmt:formatDate value="${notice.wrDt }"
		...
		<div style="display: block; text-align: center;">
			<c:if test="${paging.startPage != 1 }">
				<a
					href="noticeList.do?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
			</c:if>
			<c:forEach begin="${paging.startPage }" end="${paging.endPage }"
				...
				<c:choose>
					...
					<c:when test="${p != paging.nowPage }">
						<a
							href="noticeList.do?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
					</c:when>
				</c:choose>
			</c:forEach>
			<c:if test="${paging.endPage != paging.lastPage}">
				<a
					href="noticeList.do?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
			</c:if>
		</div>
	</div>
